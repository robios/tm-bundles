...
| Shell Script | [robios/shellscript.tmbundle](https://github.com/robios/shellscript.tmbundle) | A fork of the official Shell Script bundle: Open Terminal opens TextMate's own terminal instead of scripting Terminal.app, and a new Run Script in Terminal runs the script with a real tty. Keeps the official bundle's UUID, so installing it replaces that bundle. |
